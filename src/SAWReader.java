...
public class SAWReader {
    ...
    public List< bos.Pair<Integer,Integer>> getBlockLocs(){
        List< bos.Pair<Integer,Integer>> list = new ArrayList<>();
        for (String s: contents){
            Pattern p = Pattern.compile("block:\\s*\\((\\d+),\\s*(\\d+)\\)");
            Matcher m = p.matcher(s);
            if(m.matches()){
                list.add( new bos.Pair( Integer.parseInt(m.group(1).trim())
